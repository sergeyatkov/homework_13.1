...
def test_init(cat_1):
    ...
    assert cat_1.description == ("Смартфоны, как средство не только коммуникации,"
                                 " но и получение дополнительных функций для удобства жизни")
    assert cat_1.__products[0].price == 180000.0
    assert cat_1.__products[0].name == "Samsung Galaxy C23 Ultra"
    assert cat_1.__products[0].description == "256GB, Серый цвет, 200MP камера"
    assert cat_1.__products[0].quantity == 5
